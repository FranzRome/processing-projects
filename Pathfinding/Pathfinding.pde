...
void calculatePath(PVector a, PVector b) { 
  ...
  calculatePathIterativo (indexA, indexB, 0);
}
    ...
    for (int ix=-1; ix<=1; ix++) {
      for (int iy=-1; iy<=1; iy++) {
        ...
        if (!(ix==0 && iy==0) && tileValid(aIndex)) {
          // considera le caselle diagonali soltanto se almeno una tra quelle sopra o sotto e di lato, sono walkable
          if (!(abs(ix)==1 && abs(iy)==1 && ! (tiles[aIndex.x][currentIndex.y].walkable || tiles[currentIndex.x][aIndex.y].walkable) )) {
